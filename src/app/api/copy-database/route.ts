...
export async function POST(request: Request) {
  const { dbname, password, organizationid, email, country } =
    await request.json();
  const hashedPassword = await bcrypt.hash(password, 10);
  let message: string = "SUCCESS";
  try {
    await updateOrgainzationStatus(organizationid, dbname);
    const result = await newUser(hashedPassword, email, organizationid);
    let tmpPassword: any = hashedPassword;
    if (result != 0) {
      tmpPassword = result;
    }
