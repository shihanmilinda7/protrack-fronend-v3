  ...
  const transaction = db.transaction(() => {
    try {
      ...
      for (let i = 0; i < timelogData.length; i++) {
        ...
        const query3 = `SELECT p.projectname,pt.taskname,ti.description,ti.estimatecount,td.* FROM timelogsdetails as td 
        left join projects as p on td.projectid = p.projectid 
        left join projecttasks as pt on td.taskid = pt.taskid 
        left join taskitems as ti on td.taskitemid = ti.taskitemid 
        where td.timelogid = ${element.timelogid};`;
