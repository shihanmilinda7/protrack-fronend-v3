...
// import { compamyDatabase } from "@/db";

// const webSocket = io("http://localhost:5000"); compamyDatabase
...
const Navbar = () => {
  ...
  const ismultyorg = session?.user?.ismultyorg;
  const [isMultyOrg, setIsMultyOrg] = useState(false);
  const [isMultyOrg1, setIsMultyOrg1] = useState("SINGLE");
  ...
  useEffect(() => {
    process.env.DB_CONNECTION_STRING = session?.user?.dbname;
    setEmail(session?.user?.email);
    setOrgList(JSON.parse(session?.user?.orglist));
    setIsMultyOrg1(session?.user?.ismultyorg);

    // initializeDB();
    // callDbName();
    // compamyDatabase(session?.user?.dbname);
    // console.log("session?.user?.userid", session?.user?.dbname);
  }, [session]);

  // const callDbName = async () => {
  //   await compamyDatabase(session?.user?.dbname);
  // };
