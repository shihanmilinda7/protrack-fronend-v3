...
const StaffAddNew = (params: ParamTypes) => {
  ...
  const emailUsernameValidation = async (
    email: string,
    username: string,
    ...
    const reponse = await fetch(
      "api/staff/username-validation?email=" +
        email +
        "&username=" +
        username +
        "&staffid=" +
    ...
    try {
      ...
      if (validation == 0) {
        ...
        if (password != confirmpassword) {
          ...
          //username validation
          const tmpValidation = await emailUsernameValidation(
            email,
            username,
            0
          );
          if (tmpValidation == "NO_DATA") {
            //api call
      ...
      >
        ...
        <div className="flex items-center justify-center p-12">
          <div className="mx-auto w-full max-w-[550px] flex gap-2 flex-col">
            ...
            <div className={`${staffid ? "hidden" : "-mx-3 flex flex-wrap"}`}>
              <div className="w-full">
                <NextEmailInputField
                  label="Email"
                  value={email}
            ...
            <div
              className={`${
                staffid ? "hidden" : "-mx-3 flex flex-wrap sm:flex-nowrap gap-2"
              }`}
            >
              <div className="w-full sm:w-1/2">
                <NextTextInputField
